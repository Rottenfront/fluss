        ...
        state(
            ...
            move |width, cx| {
                ...
                canvas(move |cx, sz, vger| {
                    ...
                    let paint = vger.color_paint(BUTTON_BACKGROUND_COLOR).unwrap();
                    vger.draw_rect(
                        &RRect::from_origin_size(
                            (start_x, c.y - SLIDER_WIDTH / 2.0),
                            (sz.width() - 2.0 * r, SLIDER_WIDTH),
                            RectRadii::default(),
                        ),
                        paint,
                    );
                    let paint = vger.color_paint(AZURE_HIGHLIGHT_BACKGROUND).unwrap();
                    vger.draw_rect(
                        &RRect::from_origin_size(
                            (start_x, c.y - SLIDER_WIDTH / 2.0),
                            (x, SLIDER_WIDTH),
                    ...
                    let paint = vger.color_paint(opts.thumb).unwrap();
                    vger.draw_circle((x, c.y).into(), r, paint);
                })
                .geom(move |cx, sz, _| {
        ...
        state(
            ...
            move |height, _| {
                canvas(move |cx, sz, vger| {
                    ...
                    let paint = vger.color_paint(opts.thumb).unwrap();
                    vger.draw_circle((c.x, y).into(), SLIDER_THUMB_RADIUS, paint);
                })
                .geom(move |cx, sz, _| {
