...
impl<VT: ViewTuple + 'static, D: StackDirection + 'static> View for Stack<VT, D> {
    fn process(
        ...
        self.children.foreach_view_rev(&mut |child| {
            ...
            (*child).process(&event.offset(-offset.to_vec2()), path, cx, actions);
            path.pop();
            c -= 1;
        })
    }
    ...
    fn draw(&self, path: &mut IdPath, cx: &mut Context, vger: &mut Drawer) {
        ...
        self.children.foreach_view(&mut |child| {
            path.push(c);
            let layout_box = cx.get_layout(path);

            vger.save();

            vger.clip_rect(&layout_box.to_rounded_rect(RectRadii::default()));

            (*child).draw(path, cx, vger);
            c += 1;
    ...
    fn layout(&self, path: &mut IdPath, sz: Rect, cx: &mut Context, dcx: &mut DrawerState) -> Rect {
        ...
        match D::ORIENTATION {
            StackOrientation::Horizontal => {
                ...
                self.layout_fixed_children(path, proposed_child_size, cx, dcx, &mut child_sizes);
                ...
                sz.with_size((length, max_height))
            }
            StackOrientation::Vertical => {
                ...
                sz.with_size((max_width, length))
            }
            StackOrientation::Z => {
                let mut c = 0;
                self.children.foreach_view(&mut |child| {
                    path.push(c);
                    child.layout(path, sz, cx, dcx);
                    path.pop();
                    c += 1;
                });
                sz
    ...
    fn hittest(&self, path: &mut IdPath, pt: Point, cx: &mut Context) -> Option<ViewId> {
        ...
        self.children.foreach_view(&mut |child| {
            ...
            if let Some(h) = child.hittest(path, (pt - offset).to_point(), cx) {
                hit = Some(h)
            }
    ...
    pub fn layout_fixed_children(
        ...
        self.children.foreach_view(&mut |child| {
            ...
            if !child.is_flexible() {
                child_sizes[c as usize] = Some(
                    child
                        .layout(
                            path,
                            Rect::from_origin_size(Point::ZERO, proposed_child_size),
                            cx,
                            dcx,
                        )
                        .size(),
            ...
            if child.is_flexible() {
                child_sizes[c as usize] = Some(
                    child
                        .layout(
                            path,
                            Rect::from_origin_size(Point::ZERO, flex_size),
                            cx,
                            dcx,
                        )
                        .size(),
