...
pub trait HoverFn {
    fn call(&self, cx: &mut Context, pt: Point, inside: bool, actions: &mut Vec<Box<dyn Any>>);
}
...
impl<A: 'static, F: Fn(&mut Context, Point) -> A> HoverFn for HoverFuncP<F> {
    fn call(&self, cx: &mut Context, pt: Point, inside: bool, actions: &mut Vec<Box<dyn Any>>) {
        if inside {
...
impl<A: 'static, F: Fn(&mut Context, bool) -> A> HoverFn for HoverFunc<F> {
    fn call(&self, cx: &mut Context, _pt: Point, inside: bool, actions: &mut Vec<Box<dyn Any>>) {
