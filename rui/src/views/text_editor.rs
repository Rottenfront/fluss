...
impl TextEditorState {
    ...
    fn closest_in_range(&self, p: Point, range: std::ops::Range<usize>, rects: &[RRect]) -> usize {
        let mut d = std::f64::MAX;
        let mut closest = 0;
        for i in range {
            let dp = rects[i].center().distance(p);
            if dp < d {
                closest = i;
                d = dp;
            }
        ...
        state(TextEditorState::new, move |state, cx| {
            ...
            canvas(move |cx, rect, vger| {
                ...
                if has_focus {
                    ...
                    vger.draw_rect(
                        &RRect::from_origin_size(p, (2.0, 20.0).into(), RectRadii::default()),
                        glyph_rect_paint,
                    );
