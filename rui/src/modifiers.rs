...
pub trait Modifiers: View + Sized {
    ...
    fn drag<F: Fn(&mut Context, Vec2, GestureState, Option<MouseButton>) + 'static>(
        self,
        f: F,
    ...
    fn drag_p<F: Fn(&mut Context, Point, GestureState, Option<MouseButton>) + 'static>(
        self,
        f: F,
    ...
    fn drag_s<
        T: 'static,
        B: Binding<T>,
        F: Fn(&mut T, Vec2, GestureState, Option<MouseButton>) + 'static,
    >(
        self,
        b: B,
        f: F,
    ...
    fn hover_p<A: 'static, F: Fn(&mut Context, Point) -> A + 'static>(
        self,
        f: F,
    ...
    fn geom<F: Fn(&mut Context, Size, Vec2) + 'static>(self, f: F) -> Geom<Self, F> {
        Geom::new(self, f)
    }
    ...
    fn offset<Off: Into<Vec2>>(self, offset: Off) -> Offset<Self> {
    ...
    fn sized<Sz: Into<Size>>(self, size: Sz) -> SizeView<Self> {
    ...
    fn tap_p<A: 'static, F: Fn(&mut Context, Point, Option<MouseButton>) -> A + 'static>(
        self,
        f: F,
    ) -> Tap<Self, TapFunc<F>> {
