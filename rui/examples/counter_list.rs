        ...
        vstack((
            list(cx[counters].ids(), move |&i| {
                with_cx(move |cx| {
                    ...
                    hstack((
                        text(&format!("{}", count.get(cx))).padding(Auto),
                        ...
                        .padding(Auto),
                        button(text("remove"), move |cx| cx[counters].remove_counter(i))
                            .padding(Auto),
                    ))
                })
            }),
            text(&format!("total: {}", cx[counters].sum_counters())).padding(Auto),
            button(text("add counter"), move |cx| cx[counters].add_counter()).padding(Auto),
        ))
    })
    .run()
}
