...
pub struct LayoutArgs<'a> {
    pub sz: LocalSize,
    pub cx: &'a mut Context,
    pub text_bounds: &'a mut dyn FnMut(&str, f64, Option<f64>, FontId) -> LocalSize,
}
...
pub trait View: private::Sealed + 'static {
    ...
    fn draw(&self, path: &mut IdPath, args: &mut DrawArgs, rect: LocalRect);
    ...
    fn hit_test(&self, _path: &mut IdPath, _pt: LocalPoint, _cx: &mut Context) -> Option<ViewId> {
        None
    }
    ...
    /// returns the rectangle with physical coordinates of the view
    fn layout_box(&self, path: &mut IdPath, args: &mut LayoutArgs) -> LocalRect;
