        ...
        if self.dirty {
            ...
            view.layout(
                ...
                &mut LayoutArgs {
                    ...
                    cx: self,
                    text_bounds: &mut |s, size, max_width, font| {
                        vger.text_bounds(s, max_width, font, size)
                    },
                },
            );
            assert_eq!(path.len(), 1);
    ...
    pub fn render(
        ...
        let sz = view.layout(
            ...
            &mut LayoutArgs {
                ...
                cx: self,
                text_bounds: &mut |s, size, max_width, font| {
                    vger.state().text_bounds(s, max_width, font, size)
                },
            },
        );
        assert!(path.len() == 1);
        ...
        let mut draw_args = DrawArgs { cx: self, vger };
        view.draw(
            &mut path,
            &mut draw_args,
            LocalRect::new([0.0, 0.0].into(), window_size),
        );
