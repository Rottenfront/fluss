...
use crate::{arc::ArcAppendIter, Arc, PathEl, Point, Rect, RectRadii, Shape, Size, Vec2};
...
impl RRect {
    ...
    #[inline]
    pub fn new(x0: f64, y0: f64, x1: f64, y1: f64, radii: impl Into<RectRadii>) -> RRect {
        RRect::from_rect(Rect::new(x0, y0, x1, y1), radii)
    }
    ...
    #[inline]
    pub fn from_rect(rect: Rect, radii: impl Into<RectRadii>) -> RRect {
        let rect = rect.abs();
    ...
    #[inline]
    pub fn with_origin(self, origin: impl Into<Point>) -> RRect {
        RRect::from_origin_size(origin, self.rect.size(), self.radii)
    }

    /// Create a new `Rect` with the same origin as `self` and a new size.
    #[inline]
    pub fn with_size(self, size: impl Into<Size>) -> RRect {
        RRect::from_origin_size(self.rect.origin(), size, self.radii)
    }

    /// Create a new `Rect` with the same size and same origin with translation
    #[inline]
    pub fn translate(self, delta: impl Into<Vec2>) -> RRect {
        self.with_origin(self.origin() + delta.into())
    }
...
impl Shape for RRect {
    ...
    #[inline]
    fn as_rounded_rect(&self) -> Option<RRect> {
        Some(*self)
    }
}
...
impl Add<Vec2> for RRect {
    type Output = RRect;

    #[inline]
    fn add(self, v: Vec2) -> RRect {
        RRect::from_rect(self.rect + v, self.radii)
    }
}

impl Sub<Vec2> for RRect {
    type Output = RRect;

    #[inline]
    fn sub(self, v: Vec2) -> RRect {
        RRect::from_rect(self.rect - v, self.radii)
    }
}
...
#[cfg(test)]
mod tests {
    use crate::{Circle, Point, RRect, Rect, Shape};
    ...
    fn winding() {
        ...
        let rect = RRect::new(-10.0, -20.0, 10.0, 20.0, 0.0); // rectangle
        assert_eq!(rect.winding(Point::new(10.0, 20.0)), 1); // bottom-right corner
    }

    #[test]
    fn bez_conversion() {
        let rect = RRect::new(-5.0, -5.0, 10.0, 20.0, 5.0);
