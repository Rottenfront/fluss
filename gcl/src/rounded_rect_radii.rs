// Copyright 2021 the GCL Authors
...
/// (and Druid by extension) uses a y-down coordinate space, but GCL also
...
impl From<f64> for RectRadii {
    fn from(radius: f64) -> Self {
        RectRadii::from_single_radius(radius)
    }
}

impl From<(f64, f64, f64, f64)> for RectRadii {
    fn from(radii: (f64, f64, f64, f64)) -> Self {
        RectRadii::new(radii.0, radii.1, radii.2, radii.3)
    }
}
